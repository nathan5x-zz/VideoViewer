...
const buildThumbnails = function () {
    let docFragment = document.createDocumentFragment();

    for (let video of mediaJSON.videos) {
        let liDOM = document.createElement("li");
        let thumbnailImg = document.createElement("img");
        thumbnailImg.src = video.thumb;

        liDOM.dataset.videoSource = video.sources[0];
        liDOM.dataset.videoTitle = video.title;

        liDOM.classList.add('thumbnail');
        liDOM.append(thumbnailImg);
        docFragment.append(liDOM);
    }
    cContainer.append(docFragment);
}
...
const addEvents = function () {
    document.addEventListener('keyup', (event) => {
        ...
        if (keyName === 'ArrowLeft' || keyName === 'ArrowRight') {
            updateSelection(keyName);
        }
    });
}

const updateSelection = function (direction) {
    if (direction === 'ArrowLeft') {
        if (currentIndex > 0 && currentIndex < TOTAL_VIDEOS) {
            currentIndex = currentIndex - 1;
            selectItem(currentIndex);
        }
    } else {
        if (currentIndex >= 0 && currentIndex < TOTAL_VIDEOS - 1) {
            currentIndex = currentIndex + 1;
            selectItem(currentIndex);
        }
    }
...
const selectItem = function (index) {
    ...
    const element = cContainer.children[index];
    element.classList.add('selected');
    videoTitle.innerText = element.dataset.videoTitle;
    coverVideo.src = element.dataset.videoSource;
...
const initialize = function () {
    cContainer = document.getElementById("carouselContainer");
    videoCover = document.getElementById('coverVideo');
    videoTitle = document.getElementById('title');

    TOTAL_VIDEOS = mediaJSON.videos.length;
